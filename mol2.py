...
def read_Mol2_file(file):
    ...
    for i in range(3):
      num  = (i+1)*300000
      if i == 2:
        num = len(mol_list) - i*300000
      np.save('mol2_list%s.npy'%i, mol_list[i:num])
    #print mol_list
    atom_list = [];bond_list = []
    return mol_list
#################################################################################################################
...
#################################################################################################################
#################################################################################################################
def write_mol2(molecule,outmol2):
...
def distance2_vec(vector1,vector2):
        if (len(vector1)!=len(vector2)):
                print 'function distance(): vectors differ in length',len(vector1)
                print 'function distance(): vectors differ in length',len(vector2)
                sys.exit(1)
        distance2 = 0
...
def mol2np(mol):
    ...
    list_len = len(mol.atom_list)
    mol_atomlist = np.empty(1)
    for i in range(list_len):
        temp_atom = np.empty(3)
        temp_atom[0] = mol.atom_list[i].X
        temp_atom[1] = mol.atom_list[i].Y
        temp_atom[2] = mol.atom_list[i].Z
        print temp_atom
        mol_atomlist = np.append(mol_atomlist, temp_atom)
    t1 = time.time()
    print "transfer time used %e seconds"%(t1-t0)
    print mol_atomlist
    return mol_atomlist
def test(mol):
    t0 = time.time()
    ...
    for i in range(list_len):
        temp_atom = []
        temp_atom.append(mol.atom_list[i].X)
        temp_atom.append(mol.atom_list[i].Y)
        temp_atom.append(mol.atom_list[i].Z)
        print temp_atom
        mol_atomlist.append(temp_atom)
    ...
    return mol_atomlist
#################################################################################################################
def correct_RMSD(moli, molj):
    

    """Returns RMSD between 2 sets of [nx3] numpy array"""

    assert(moli.shape[1] == 3)
    assert(moli.shape == molj.shape)

    n_vec = np.shape(moli)[0]
    correlation_matrix = np.dot(np.transpose(moli), molj)
    v, s, w = np.linalg.svd(correlation_matrix)
    is_reflection = (np.linalg.det(v) * np.linalg.det(w)) < 0.0
    if is_reflection:
      s[-1] = - s[-1]
    E0 = sum(sum(moli * moli)) + \
         sum(sum(molj * molj))
