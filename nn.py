...
#compute distance 
  ...
  nlRes = []
  mov_nl_coefs = mov_model.nn_array.coefs()
...
#cc process pool
  ...
  for n in range(0,len(cc_result)):
    sortedNnRes += cc_result[n]
  # sortedNnRes = sorted(sortedNnRes, key=operator.itemgetter(2), reverse=True)[:args.num_nn_round]
  tnlm2 = time.time()

#merge chi to cc arr
  tmerge1 = time.time()
  for i in range(totalSize):
    indx = sortedNnRes[i][0]
