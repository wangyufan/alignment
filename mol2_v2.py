...
def read_Mol2_lines(lines, startline):
    ...
    for lnum in range(startline, len(lines)):
        ...
        if ((len(linesplit) >= 9) and (flag_atom)):
            ...
            Q = linesplit[8]
            temp_atom = atom(X, Y, Z, Q, atom_type, atom_name, atom_num,
                             res_num, res_name)
        ...
        if (len(linesplit) == 1):
            if (linesplit[0] == "@<TRIPOS>MOLECULE"):
                if flag_frist_mol:
                    ...
                    #print header2
                    ID_heavy_atoms(atom_list)
                    data = Mol(header2, Name, atom_list, bond_list,
                               residue_list)
                    mol_list.append(data)
                    atom_list = []
                    bond_list = []
...
def write_mol2(molecule, filename):
    ...
    outmol2.write("%-5d %-5d %-5d 0     0\n" %
                  (len(molecule.atom_list), len(molecule.bond_list),
    ...
    for j in range(0, len(molecule.atom_list)):
        ...
        outmol2.write(
            ...
            (atom_dic[molecule.atom_list[j].num], molecule.atom_list[j].name,
             molecule.atom_list[j].X, molecule.atom_list[j].Y,
             molecule.atom_list[j].Z, molecule.atom_list[j].type,
             resid_dic[molecule.atom_list[j].resnum],
    ...
    for m in range(0, len(molecule.bond_list)):
        outmol2.write("%-5d %-5d %-5d %s\n" %
                      (count, atom_dic[molecule.bond_list[m].a1_num],
                       atom_dic[molecule.bond_list[m].a2_num],
                       molecule.bond_list[m].type))
        count = count + 1
        ...
        if atom.type == 'H':
            ...
            for bond in molecule.bond_list:
                ...
                if (i == bond.a1_num):
                    ...
                    break  # hydrogens are only attached to on atom.
            if hflag:
                if molecule.atom_list[j - 1].type in [
                        'C.1', 'C.3', 'C.2', 'C.ar', 'C.cat'
                ]:
...
def distance2(atom1, atom2):
    return (atom1.X - atom2.X)**2 + (atom1.Y - atom2.Y)**2 + (
        atom1.Z - atom2.Z)**2
