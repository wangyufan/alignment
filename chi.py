...
align{
  dbnum = 2
  .type=int
  .help="cut a DB into dbnum lighter dbs"
...
def run(args):
  targetfile = os.path.join(os.path.split(sys.path[0])[0],"chi4")
...
#compute nlm_cc
  ...
  for i in range(nlNum):
    ...
    cc = align_obj.get_cc()
    nlmRes.append((sortedNlRes[i][0], sortedNlRes[i][2], cc, sortedNlRes[i][1]))
  sortedNlmRes = nlmRes
  tnlm2 = time.time()
